...
class Ec2_cloud(object):
  ...
  def __init__(self,ec2_access_key, ec2_secret_key, ec2_url, s3_url):
    # for now, dev
    # for test purpose. this is using hardcoded values.
    # later will be databbase driven
    self.RACKSPACE_USER = 'iplant'
    self.RACKSPACE_KEY = '****'
    #self.euca = Euca2ool()
  ...
  def getUserProfile(self,req):
    ...
    #
    return atmo_util.jsoner("\"success\"","\"\"","[%s]" % "null")

  def getImageList(self, req):  
    from libcloud.compute.types import Provider
    from libcloud.compute.providers import get_driver
    Driver = get_driver(Provider.RACKSPACE)
    conn = Driver(self.RACKSPACE_USER, self.RACKSPACE_KEY)
    ...
    for image in images:
      image_json_string = image_json_string + """,{
          ...
          "image_ownerid" : null ,
          "image_state" : null ,
          "image_is_public" : null ,
          "image_product_codes" : null ,
          "image_architecture" : null ,
          "image_type" : null ,
          "image_ramdisk_id" : null ,
          "image_kernel_id" : null
        }""" % (image.name, image.name)
    #return atmo_util.jsoner("\"success\"","\"\"","%s" % json.dumps(my_available_image))
    return atmo_util.jsoner("\"success\"","\"\"","[%s]" % image_json_string[1:])
  ...
  def getInstanceList(self, req) :
    ...
    for node in nodes:
      ...
      queued_instance_json_string = queued_instance_json_string + """
      {
            ...
            "instance_kernel" : "" ,
            "instance_ramdisk" : ""
          }, """ % (instance_num, name, uuid, public_ip[0], state )
      
    return atmo_util.jsoner("\"success\"","\"\"", "["+queued_instance_json_string[0:-2]+"]")
  ...
  def getRunningInstancesTree(self, req):
    ...
    return_json_str = "[]"
    return atmo_util.jsoner("\"success\"","\"%s\"",return_json_str)
  ...
  def launchInstance(self, req) :
    if req.method == "POST":
      ...
      instance_size = req.POST['instance_size']
      ...
      node = conn.create_node(name=instance_name, image=images[0], size=sizes[0])
      


      return atmo_util.jsoner("\"success\"","\"\"","\"%s\""  % node.uuid)
    else:
...
#      return atmo_util.jsoner("\"fail\"","\"expecting post method but not received it\"","\"\"")
  ...
  def getVmTypes(self,req) :
    ...
    r = ""
    for type in conn.list_sizes() : r += """{"name" : "%s" , "cpus" : "%s" , "memory" : "%s" , "disc" : "%s"},""" % (type.name,"-",type.ram,type.disk)
    return atmo_util.jsoner("\"success\"","\"\"", "["+r[:-1]+"]")
